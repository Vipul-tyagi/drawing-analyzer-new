...
class SmartAnimator:
    ...
    def __init__(self):
        ...
        self.story_timing = {
            ...
            'evening': {'sun': 8, 'cloud': 2, 'animal': 4, 'bird': 6},
    ...
    def create_coordinated_animation(self, element_clips_data, user_story=None):
        ...
        element_clips_data.sort(key=lambda x: layer_order.get(x['classification']['layer'], 1))
        
        for i, element_data in enumerate(element_clips_data):
            ...
            try:
                ...
                if animated_clip:
                    coordinated_clips.append(animated_clip)
                    print(f"✅ Coordinated {classification['label']} animation")
                
            except Exception as e:
                print(f"⚠️ Failed to coordinate {classification['label']}: {e}")
                # Fallback to simple positioning
                x, y = element_info['center']
                simple_clip = element_clip.set_position((x, y))
                coordinated_clips.append(simple_clip)
        
        return coordinated_clips
    ...
    def _create_background_coordination(self, element_clip, element_info, classification, user_story, scene_time):
        ...
        if object_label == 'sun':
            ...
            return self._create_seamless_rainbow_animation(element_clip, element_info)
        else:
    ...
    def _create_foreground_coordination(self, element_clip, element_info, classification, user_story, scene_time, index):
        ...
        if object_label in ['animal', 'dog', 'cat', 'person', 'child']:
            ...
            return self._create_seamless_swimming_animation(element_clip, element_info, story_cues)
        elif object_label == 'boat':
            return self._create_seamless_floating_animation(element_clip, element_info, story_cues)
        else:
